...
bool debug_module_t::perform_abstract_command()
{
  ...
  if (cmdtype == DM_COMMAND_CMDTYPE_ACCESS_REGISTER)
    return perform_abstract_register_access();

  if (cmdtype == DM_COMMAND_CMDTYPE_ACCESS_MEMORY)
...
bool debug_module_t::perform_abstract_memory_access() {
  ...
  bool handled = is_write ? handle_memory_write(xlen, aamsize, offset)
  ...
  abstractcs.cmderr = !handled ? CMDERR_NOTSUP : CMDERR_NONE;
  return true;
}

using func = uint32_t (*)(unsigned rd_src, unsigned base, uint16_t offset);
static constexpr std::array<func, 4> lx = {&lb, &lh, &lw, &ld};
static constexpr std::array<func, 4> sx = {&sb, &sh, &sw, &sd};

static inline unsigned idx(unsigned xlen)
{
  return field_width(xlen) - 3;
}
...
bool debug_module_t::handle_memory_read(size_t xlen, unsigned aamsize, unsigned &offset)
{
...
bool debug_module_t::handle_memory_write(size_t xlen, unsigned aamsize, unsigned &offset)
{
...
{
  write32(debug_abstract, offset++, lx[idx(xlen)](S1, ZERO, arg(xlen, 1)));
  write32(debug_abstract, offset++, addi(S1, S1, std::pow(2u, aamsize)));
